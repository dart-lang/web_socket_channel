...
class _WebSocketProtocolTransformer
    extends StreamTransformerBase<List<int>, dynamic /*List<int>|_WebSocketPing|_WebSocketPong*/ >
  ...
  void _unmask(int index, int length, Uint8List buffer) {
    ...
    if (length >= BLOCK_SIZE) {
      ...
      if (blockCount > 0) {
        ...
        Int32x4List blockBuffer = new Int32x4List.view(buffer.buffer, index, blockCount);
...
class _WebSocketTransformerImpl extends StreamTransformerBase<HttpRequest, WebSocket>
  ...
  static Future<WebSocket> _upgrade(
      HttpRequest request, _ProtocolSelector _protocolSelector, CompressionOptions compression) {
    ...
    Future<WebSocket> upgrade(String protocol) {
      ...
      return response.detachSocket().then<WebSocket>(
          (socket) => new _WebSocketImpl._fromSocket(socket, protocol, compression, true, deflate));
    }
    ...
    if (protocols != null && _protocolSelector != null) {
      ...
      return new Future<String>(() => _protocolSelector(protocols)).then<String>((protocol) {
  ...
  static _WebSocketPerMessageDeflate _negotiateCompression(
    ...
    if (compression.enabled && hv.value == _WebSocketImpl.PER_MESSAGE_DEFLATE) {
      ...
      var serverNoContextTakeover = (hv.parameters.containsKey(_serverNoContextTakeover) &&
  ...
  void _ensureDecoder() {
    if (decoder == null) {
      decoder = new RawZLibFilter.inflateFilter(
          windowBits: serverSide ? clientMaxWindowBits : serverMaxWindowBits, raw: true);
    }
  }
  ...
  void _ensureEncoder() {
    if (encoder == null) {
      encoder = new RawZLibFilter.deflateFilter(
          windowBits: serverSide ? serverMaxWindowBits : clientMaxWindowBits, raw: true);
    }
  }
...
class _WebSocketOutgoingTransformer extends StreamTransformerBase<dynamic, List<int>>
  ...
  Stream<List<int>> bind(Stream stream) {
    return new Stream<List<int>>.eventTransformed(stream, (EventSink<List<int>> eventSink) {
      if (_eventSink != null) {
        throw new StateError("WebSocket transformer already used");
      }
      _eventSink = eventSink;
  ...
  void addFrame(int opcode, List<int> data) {
    createFrame(
            ...
            _deflateHelper != null &&
                (opcode == _WebSocketOpcode.TEXT || opcode == _WebSocketOpcode.BINARY))
        .forEach((e) {
      _eventSink.add(e);
    });
  }
    ...
    if (mask) {
      ...
      if (data != null) {
        ...
        if (blockCount > 0) {
          ...
          Int32x4 blockMask = new Int32x4(mask, mask, mask, mask);
          Int32x4List blockBuffer = new Int32x4List.view(list.buffer, 0, blockCount);
          for (int i = 0; i < blockBuffer.length; i++) {
            blockBuffer[i] ^= blockMask;
          }
        }
...
class _WebSocketConsumer implements StreamConsumer {
  ...
  _ensureController() {
    ...
    _controller = new StreamController(
        sync: true, onPause: _onPause, onResume: _onResume, onCancel: _onListen);
    var stream = _controller.stream.transform(new _WebSocketOutgoingTransformer(webSocket));
...
class _WebSocketImpl extends Stream with _ServiceObject implements WebSocket {
  ...
  static Future<WebSocket> connect(
    ...
    return _httpClient.openUrl("GET", uri).then((request) {
      ...
      _WebSocketPerMessageDeflate deflate = negotiateClientCompression(response, compression);

      return response.detachSocket().then<WebSocket>((socket) =>
          new _WebSocketImpl._fromSocket(socket, protocol, compression, false, deflate));
    });
  }
