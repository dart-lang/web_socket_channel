...
class _HttpHeaders implements HttpHeaders {
  ...
  void set persistentConnection(bool persistentConnection) {
    ...
    if (persistentConnection) {
      if (protocolVersion == "1.1") {
        ...
        if (_contentLength == -1) {
          throw new HttpException("Trying to set 'Connection: Keep-Alive' on HTTP 1.0 headers with "
              "no ContentLength");
        }
  ...
  void set contentLength(int contentLength) {
    ...
    if (protocolVersion == "1.0" && persistentConnection && contentLength == -1) {
      throw new HttpException("Trying to clear ContentLength on HTTP 1.0 headers with "
  ...
  static String _validateField(String field) {
    for (var i = 0; i < field.length; i++) {
      if (!_HttpParser._isTokenChar(field.codeUnitAt(i))) {
        throw new FormatException("Invalid HTTP header field name: ${json.encode(field)}");
      }
    }
  ...
  static _validateValue(value) {
    ...
    for (var i = 0; i < value.length; i++) {
      if (!_HttpParser._isValueChar(value.codeUnitAt(i))) {
        throw new FormatException("Invalid HTTP header field value: ${json.encode(value)}");
      }
    }
    return value;
  }
...
class _HeaderValue implements HeaderValue {
  ...
  static _HeaderValue parse(String value,
      {parameterSeparator: ";", valueSeparator: null, preserveBackslash: false}) {
    // Parse the string.
...
class _ContentType extends _HeaderValue implements ContentType {
  ...
  _ContentType(String primaryType, String subType, String charset, Map<String, String> parameters)
...
class _Cookie implements Cookie {
  ...
  static String _validateName(String newName) {
    ...
    for (int i = 0; i < newName.length; i++) {
      ...
      if (codeUnit <= 32 || codeUnit >= 127 || separators.indexOf(newName[i]) >= 0) {
        throw new FormatException(
            "Invalid character in cookie name, code unit: '$codeUnit'", newName, i);
      }
    }
    return newName;
  }
  ...
  static String _validateValue(String newValue) {
    ...
    for (int i = start; i < end; i++) {
      ...
      if (!(codeUnit == 0x21 ||
        ...
        throw new FormatException(
            "Invalid character in cookie value, code unit: '$codeUnit'", newValue, i);
      }
    }
    return newValue;
  }
